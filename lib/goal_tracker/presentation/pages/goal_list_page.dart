        ...
        child: BlocBuilder<GoalBloc, GoalState>(
          builder: (context, state) {
            if (state is GoalLoading) {
              ...
              final sortedGoals = sortListByDate(goals, (goal) => goal.targetDate);
              final byYear = groupByYear<Goal>(sortedGoals, (goal) => goal.targetDate);
              final undated = filterItemsWithNullDate<Goal>(sortedGoals, (goal) => goal.targetDate);
              ...
              return ListView(
                ...
                children: [
                  YearTimelineList<Goal>(
                    yearKeys: yearKeys,
                    itemsByYear: byYear,
                    itemBuilder: (context, goal) => buildGoalCard(context, goal, state.allMilestonesMap),
                  ),
                          ...
                          for (final goal in undated)
                            Padding(
                              ...
                              child: SharedCard(
                                  ...
                                  Navigator.of(context).push(
                                    MaterialPageRoute(
                                      builder: (_) => MultiBlocProvider(
                                        providers: [
                                          BlocProvider.value(value: context.read<MilestoneBloc>()),
                                ...
                                onActionPressed: () => openAddMilestonePopup(context, goal),
                                actionListItems: goal.milestoneIds.isNotEmpty
                                    ? buildLimitedMilestonesList(goal.milestoneIds, state.allMilestonesMap)
                                    : null,
                              ),
                            ),
                        ],
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: SharedButton(
                  label: "Add Goal",
                  icon: Icons.add,
                  onPressed: () => openAddGoalPopup(context),
                ),
              ),
            ),
...
List<String> buildLimitedMilestonesList(List<String> milestoneIds, Map<String, Milestone> allMilestones) {
  final milestones = milestoneIds
      .map((id) => allMilestones[id])
      .whereType<Milestone>()
      .toList();

  final sortedMilestones = sortListByDate(milestones, (milestone) => milestone.targetDate);
...
Widget buildGoalCard(BuildContext context, Goal goal, Map<String, Milestone> allMilestonesMap, ) {
  return SharedCard(
    icon: Icons.flag,
    title: goal.title,
    onCardTap: () {
      ...
      // Navigator.of(context).push(
      //   MaterialPageRoute(
      //     builder: (_) => MilestoneListPage(goalId: goal.id),
      //   ),
      // );

    },
    subtitle2Icon: Icons.calendar_today,
    subtitle2: goal.targetDate != null ? DateFormat('dd/MM/yyyy').format(goal.targetDate!) : null,
    actionButtonText: "ADD MILESTONE",
    onActionPressed: () => openAddMilestonePopup(context, goal),
    actionListItems: goal.milestoneIds.isNotEmpty
        ? buildLimitedMilestonesList(goal.milestoneIds, allMilestonesMap)
        : null,
  );
}
